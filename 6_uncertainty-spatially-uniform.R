...
# Generate perturbation sets per variable, sampled from normal distributions of mean = 0 and standard deviation equal to 0.1 * the standard deviation of raw basin-aggregated distribution
Cptb <- rnorm(runs, mean = 0, sd = 0.1*sqrt(var(c_df$cons)))
Qptb <- rnorm(runs, mean = 0, sd = 0.1*sqrt(var(c_df$roff)))
Tptb <- rnorm(runs, mean = 0, sd = 0.1*sqrt(var(c_df$tws)))
Eptb <- rnorm(runs, mean = 0, sd = 0.1*sqrt(var(c_df$efn)))
Vptb <- rnorm(runs, mean = 0, sd = 0.1*sqrt(var(c_df$vsi)))
Aptb <- rnorm(runs, mean = 0, sd = 0.1*sqrt(var(c_df$ac)))

# Combine these perturbation sets into a single dataframe, for populating throughout the sensitivity analysis
...
for (i in 1:nrow(s_df)) {
  ...
  for (k in 1:nrow(t_df)) {
    t_df$cons[k] <- max(t_df$cons[k] + Cptb[i], 0) # Ensure no negative withdrawal rates
    t_df$roff[k] <- max(t_df$roff[k] + Qptb[i], 0.1) # Ensure there is are no 0 streamflow values
    t_df$tws[k]  <- t_df$tws[k]  + Tptb[i]
    t_df$efn[k]  <- max(min(t_df$efn[k] + Eptb[i], 1), 0) # Ensure staying within 0-1 range
    t_df$vsi[k]  <- max(min(t_df$vsi[k] + Vptb[i], 1), 0) # Ensure staying within 0-1 range
    t_df$ac[k]   <- max(min(t_df$ac[k] + Aptb[i], 1), 0)  # Ensure staying within 0-1 range
  }
  ...
  t_df$overallsens <- 1 - ((1-t_df$ecosens)*(1-t_df$ac))
  t_df$overallprod <- t_df$cind * t_df$overallsens
  
  # Store perturbations applied as ratio of variable's standard deviation
  s_df$Cptb[i] <- Cptb[i]/sqrt(var(c_df$cons))
  s_df$Qptb[i] <- Qptb[i]/sqrt(var(c_df$roff))
  s_df$Tptb[i] <- Tptb[i]/sqrt(var(c_df$tws))
  s_df$Aptb[i] <- Aptb[i]/sqrt(var(c_df$ac))
  s_df$Eptb[i] <- Eptb[i]/sqrt(var(c_df$efn))
  s_df$Vptb[i] <- Vptb[i]/sqrt(var(c_df$vsi))
...
ggsave(filename = "C:/Users/xande/Desktop/sens_scatter.pdf", plot = sensplot,
...
ggsave(filename = "C:/Users/xande/Desktop/threshold_frequency_uniformpert.pdf", 
...
ggsave(filename = "C:/Users/xande/Desktop/hotspot_frequency_uniformpert.pdf", 
       plot = last_plot(), device = 'pdf', width = 190/1.5, height = 100/1.5, 
       units = 'mm', dpi = 400)
